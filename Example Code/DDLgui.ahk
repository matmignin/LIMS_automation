...
DDLContent =
(Join|
Big, Empty Plugin`t`t`t`t- Doesn't do anything, just takes up space and helps test scrollable GUIs
Code Runner`t`t`t`t`t- Runs AHK code when you press an Input Button|
GameBind Toogler`t`t`t`t- Makes binding outputs to game functions easier by introducing a delay
Profile Switcher`t`t`t`t`t- Changes to a named profile whem you hit an Input Button.
Remapper (Mouse Axis To Joystick Axis)`t`t- Converts mouse input delty information into two joystick axes
)
Gui, Margin, 10, 10
