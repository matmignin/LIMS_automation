...
GetRotationTable(){
	Global
		...
		;XL.Sheets("Ingredients").select
		XL.Visible := 1
		;Rotation:=[]
		Chemicals:=[]
		RotationOrder:=[]
	while (Xl.Range("K" . A_Index).Value != "") {
		RotationOrder[A_index]:=Xl.Range("J" . A_Index+1).Text
		Chemicals[A_index]:=Xl.Range("K" . A_Index+1).Text
		Total_rows:=A_index
		Cycle:=A_Index-1
		}	
	loop, %Cycle%,
