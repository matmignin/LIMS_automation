...
clipChange(type){
  ...
  sleep 50
    ...
    ; if winactive("ahk_exe WFICA32.EXE"){
      ; clip.Department()
      /* ;; Trying to replace the value line of customer list
        StringReplace, Clipboard, Clipboard,Value,111Skin Limited
        RegExReplace(clipboard, "s)Value.111Skin Limited","111Skin Limited")
        sleep 200
        msgbox % Clipboard
        */
      ; }
...
SingleRegex(){
    global Batch, Product, lot, coated, sampleid, Products, CurrentCodes, ConnectedProducts, regexproduct, regexBatch, RegexLot, RegexCoated
    ConnectedProducts:=[]
    ConnectedProduct:=
      Haystack:=Clipboard
    sleep      20
...
Regex(Category:=""){
    global Batch, Batch0, Product, Product0, Product1, Product2, Product3, lot, coated, sampleid, analytical,micro,retain,physical,CTphysical,CTretain, products, Department, CurrentCodes, RegexProduct, RegexBatch, RegexLot, RegexCoated
    if !Category
      Haystack:=Clipboard
    else
      Haystack:=Category
    sleep      20
      ...
      If cProduct && cBatch && clot || cCoated
        ; FileAppend, %cProduct% %cBatch% %clot% %cCoated%`n, data\CurrentCodes.txt
      If cProduct {
        GuiControl,Varbar:Text, Product, %cProduct%
        Product:=cProduct
