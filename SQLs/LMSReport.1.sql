SELECT DISTINCT 
  ...
  (SELECT COUNT(*) FROM (SELECT ingredient_group, ingredient, result 
  FROM (SELECT 
  i.generic2 AS ingredient_group,
  i.ingredientid AS ingredient,
  CASE
...
WHERE $X{IN, ps.batchnumber, PARAM_BN} AND ps.deletion = 'N'
...
WHERE ingredient NOT IN ('Pesticides', 'Residual Solvents') AND ingredient_group != 'Heavy Metals' AND result = 'Not Tested')) AS count_bovmtdpc,
  (SELECT COUNT(1) 
